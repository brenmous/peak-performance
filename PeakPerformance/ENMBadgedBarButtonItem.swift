...
let kENMDefaultPadding: CGFloat = 2.0
let kENMDefaultMinSize: CGFloat = 8.0
let kENMDefaultOriginX: CGFloat = 0.0
let kENMDefaultOriginY: CGFloat = -6.0
...
class ENMBadgedBarButtonItem: UIBarButtonItem {
    ...
    var badgeBackgroundColor: UIColor = UIColor.init(red: 148/255, green: 87/255, blue: 152/255, alpha: 1) {
        didSet {
    ...
    var badgeFont: UIFont = UIFont.systemFont(ofSize: 8.0){
        didSet {
            refreshBadgeLabelProperties()
        }
    }
...
extension ENMBadgedBarButtonItem {
    ...
    func createBadgeLabel() -> UILabel {
        let frame = CGRect(x: badgeOriginX, y: badgeOriginY, width: 10, height: 10)
